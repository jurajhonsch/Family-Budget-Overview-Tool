...
parser = argparse.ArgumentParser(prog="Family Budget Overview Tool",
...
def index():
    limit = request.args.get(
        "limit", default=config["display"]["limit"], type=int)
    args = {
        "limit": limit,
    ...
    with sql.connect(config["server"]["database"]) as con:
        ...
        if args["min_amount"] > args["max_amount"] and args["max_amount"] > 0:
            args["min_amount"], args["max_amount"] = args["max_amount"], args["min_amount"]
        ...
        query = "select uuid, name, amount, type, category, date, comment from data" + \
    ...
    args.update({"all_data": data, "has_data": has_data,
...
def chart():
    ...
    chart_type = "Epoch" if chart_type == "epoch" or chart_type == "" else "Compare"

    type = request.args.get("type", default="expense")
    from_date = request.args.get("from-date")
    to_date = request.args.get("to-date")

    return render_template("chart.html", chart_type=chart_type, type=type, from_date=from_date, to_date=to_date)
...
def chart_data():
    ...
    try:
        ...
        options = {"theme_primary": config["theme"]["dark"],
                   "theme_medium": config["theme"]["primary"], "font_size": config["theme"]["font_size"]}
        ...
        if chart_type == "epoch" or chart_type == "":
            ...
            with sql.connect(config["server"]["database"]) as con:
                ...
                if from_date:
                    ...
                    cur.execute(
                        "select date from data order by date asc limit 1")
                ...
                if to_date:
                    ...
                    cur.execute(
                        "select date from data order by date desc limit 1")
                ...
                query = " where " + " and ".join(query) if query else ""
                query = "select category, amount, date from data" + query + " order by date asc"
            ...
            months = [datetime.datetime.fromisoformat(str(x).rsplit("-", 1)[0] + "-01") for x in date_range(
                start=from_date, end=datetime.datetime.fromisoformat(to_date) + relativedelta(months=1), freq="m")]
            ...
            for category, amount, date in cur:
                ...
                if category not in catsIndex:
                    ...
                    color = config["display"]["colors"][config["display"]["categories"].index(
                        category) % len(config["display"]["colors"])]

                    data.append({"label": category, "backgroundColor": color,
                                 "borderColor": color, "data": [0]})
            ...
            status = {"status": "ok", "data": {
                "data": {"labels": months, "datasets": data}, "options": options}}
...
def updateRow():
    ...
    try:
        ...
        with sql.connect(config["server"]["database"]) as con:
            ...
            cur.execute(
                "update data set name=:name, amount=:amount, category=:category, date=:date, comment=:comment where uuid=:uuid", data)
...
def addRow():
    ...
    try:
        ...
        with sql.connect(config["server"]["database"]) as con:
            ...
            cur.execute(
                "insert into data (uuid, name, amount, type, category, date, comment) values (:uuid, :name, :amount, :type, :category, :date, :comment)", data)
...
@app.route("/theme.css")
def theme():
    return Response(render_template("theme.css", **config["theme"]), mimetype="text/css")
...
def server_thread():
    waitress.serve(app, host=config["server"]
